...
Delegate to a subagent only when the task is independent and bounded and delegation materially improves context isolation, parallelism, specialised focus, or verification. Complete small, tightly coupled, sequential, or shared-context work directly. Prefer parallel delegation for read-heavy exploration, testing, triage, and independent review; avoid parallel edits unless ownership is clearly separated or the work is isolated. Give each subagent one clear responsibility, a self-contained prompt, only the context and tools it needs, an explicit deliverable, and a verification criterion. Review, verify, and integrate its output before treating the work as final.

When delegation is justified, use a capable general-purpose model by default. Do not select a weaker model solely to reduce cost; use a faster or lower-cost model only for low-risk work whose output can be verified cheaply and reliably. Escalate to the highest-capability model only when the parent agent identifies a concrete capability gap, the task requires unusually difficult or ambiguous reasoning, or the default subagent fails verification. Do not select the highest-capability model merely because the parent uses it or it is available. Subagents must not spawn further subagents unless the parent explicitly authorises a genuinely independent second-level task.
